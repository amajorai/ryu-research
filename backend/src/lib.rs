...
//! direction-aware scalar metric parsed from stdout, and a keep-if-improved-else-
...
//!
//! - the **`/api/research/*` data path** ([`routes`]) — a thin JSON proxy for
//!   status, campaign history, RLM analysis, workspace init, and legacy ledger, and
//! - the **`research.*` MCP tool contract** ([`tool_specs`] + [`dispatch`]) — the
//!   the original 8 workspace tools plus 4 durable campaign/reasoning tools, served
//!   by the crate's own stdio MCP server
//!   ([`mcp_stdio`], `ryu-research mcp`) and reached through the manifest's
//!   `mcp_servers` entry. Core used to host these tools itself, in a hardcoded
//!   `sidecar/mcp/research.rs` provider; that app-specific Core module is gone.
//!
...
//!
//! The Python engine lifecycle is owned by this satellite's [`EngineSupervisor`].
//! Core only starts the generic Rust app sidecar and proxies its declared routes;
//! no Research-specific Python manager or Core API is required.
...
pub use engine_runtime::EngineSupervisor;
...
/// Resolve the private bearer shared by this Rust client and the Python engine.
/// It is intentionally separate from Core's extension token.
pub fn research_engine_token() -> Option<String> {
    if let Ok(token) = std::env::var("RYU_RESEARCH_ENGINE_TOKEN") {
        let token = token.trim();
        if !token.is_empty() {
            return Some(token.to_owned());
        }
    }
    std::fs::read_to_string(research_engine_token_path())
        .ok()
...
#[must_use]
pub fn research_engine_token_path() -> std::path::PathBuf {
    if let Some(root) = std::env::var_os("RESEARCH_WORKSPACES") {
        let root = std::path::PathBuf::from(root);
        if !root.as_os_str().is_empty() {
            return root.join(ENGINE_TOKEN_FILE);
        }
    }
    ...
    ryu_dir.join("research-workspaces").join(ENGINE_TOKEN_FILE)
}

pub(crate) fn default_ryu_dir() -> std::path::PathBuf {
    ryu_sidecar_runtime::ryu_dir()
}

/// Base URL of the Python autoresearch engine the proxy ([`routes`]) and the
/// `research.*` tools ([`dispatch`]) forward to. Defaults to the loopback
/// `RESEARCH_ADDR` (`127.0.0.1:8087`) owned by this app's supervisor.
///
...
pub fn research_base_url() -> String {
    ...
    let port = std::env::var("RESEARCH_PORT")
        .ok()
        .and_then(|value| value.trim().parse::<u16>().ok())
        .unwrap_or(RESEARCH_PORT);
