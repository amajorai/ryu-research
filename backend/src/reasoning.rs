...
impl HistorySnapshot {
    fn parse(value: Value) -> Result<Self> {
        ...
        if !is_sha256_digest(&history_digest) {
            bail!("research history_digest must use the sha256:<64-hex> wire format");
        }
...
impl VerifiedRlm {
    fn parse(value: &Value, expected_digest: &str, source_prefix: &str) -> Result<Self> {
        ...
        if !is_sha256_digest(&input_digest) {
            bail!("RLM input_digest must use the sha256:<64-hex> wire format");
        }
...
mod tests {
    ...
    fn verify_rejected(response: Value, expected: &str) {
        ...
        let error = VerifiedRlm::parse(&response, expected_digest, "campaign:c-1/")
            .err()
            .expect("response should be rejected");
        assert!(error.to_string().contains(expected), "got {error:#}");
        assert!(VerifiedRlm::parse(&valid_rlm(), expected_digest, "campaign:c-1/").is_ok());
    }
    ...
    fn rejects_digest_mismatch_wrong_source_and_no_recursion() {
        let mut unprefixed = valid_rlm();
        unprefixed["input_digest"] = json!("digest-1");
        verify_rejected(unprefixed, "wire format");
        ...
        let app = Router::new()
            .route(
                ...
                get(move |Path(id): Path<String>| async move {
                    ...
                    Json(json!({
                        "document": "history",
                        "history_digest": "sha256:1111111111111111111111111111111111111111111111111111111111111111",
                        "attempt_count": 3,
