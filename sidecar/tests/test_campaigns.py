...
class CampaignStoreTests(unittest.TestCase):
    ...
    def test_run_records_attempts_and_enforces_the_winner(self) -> None:
        ...
        self.assertEqual(
            "improvement-1",
            initialized["campaign"]["improvement_run_id"],
        )
    ...
    def test_improvement_run_id_rejects_path_like_values(self) -> None:
        with self.assertRaisesRegex(ValueError, "improvement_run_id"):
            workspaces.init_workspace("toy", improvement_run_id="../escape")
...
"""
    ...
    def test_history_digest_uses_the_rlm_sha256_wire_format(self) -> None:
        campaign_id = workspaces.init_workspace("toy")["campaign_id"]
        history = campaigns.history_projection(campaign_id)
