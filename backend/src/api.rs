...
pub struct InitWorkspaceBody {
    ...
    /// Optional shared recursive-improvement run id. Research stores the
    /// correlation on the durable campaign; the improvement owner retains the
    /// promotion and rollback decision.
    #[schema(rename = "improvementRunId")]
...
pub async fn research_init_workspace(
    ...
    let improvement_run_id = body
        ...
        .map(str::to_owned);
    if let Some(id) = improvement_run_id.as_deref() {
        if let Err(error) = ImprovementRunRef::new(id.to_owned()) {
            return (
                StatusCode::BAD_REQUEST,
                Json(json!({
                    "error": "invalid improvementRunId",
                    "code": "invalid_improvement_run_id",
                    "details": error.to_string(),
                })),
            );
        }
        body["improvementRunId"] = json!(id);
        body.as_object_mut()
            .map(|object| object.remove("improvement_run_id"));
    }
...
mod tests {
    ...
    async fn init_workspace_starts_sidecar_and_proxies_body() {
        ...
        assert_eq!(body["echo"]["improvementRunId"], json!("improvement-1"));
        // Lazy-start was attempted exactly once.
        assert_eq!(host.start_calls.load(Ordering::SeqCst), 1);
    }

    #[tokio::test]
    async fn init_workspace_rejects_invalid_improvement_reference_before_starting() {
        let _g = UpstreamGuard::set(Some("127.0.0.1:1"));
        let host = FakeHost::new(true, false);
        let resp = research_init_workspace(
            State(ctx(host.clone())),
            Json(json!({ "experiment": "toy", "improvementRunId": "../escape" })),
        )
        .await
        .into_response();
        let (code, body) = read(resp).await;
        assert_eq!(code, StatusCode::BAD_REQUEST);
        assert_eq!(body["code"], json!("invalid_improvement_run_id"));
    ...
    fn post_routes_document_their_request_body() {
        ...
        let resolved = if let Some(reference) = schema.get("$ref") {
            let name = reference
                .as_str()
                .and_then(|value| value.rsplit('/').next())
                .expect("schema ref name");
            wire.pointer(&format!("/components/schemas/{name}"))
                .expect("schema ref target")
        } else {
            &schema
        };
        assert!(
            resolved
                ...
                .is_some_and(|properties| properties.contains_key("improvementRunId")),
            "the improvement correlation must be part of the documented init contract: {resolved}"
        );
    }
