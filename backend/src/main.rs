...
//! it instead serves the 8 `research__*` tools as a JSON-RPC 2.0 MCP server over
...
async fn main() -> Result<()> {
    // `ryu-research mcp` — serve the `research__*` tools over stdin/stdout instead
...
mod tests {
    ...
    /// load-bearing part: Core's registry ids are `<key>__<tool>`, so anything but
    /// `SERVER_NAME` silently renames all 8 tools out from under the workflow
    /// templates that call `research__run` / `research__init_workspace`.
    #[test]
    fn manifest_mcp_declaration_matches_this_binary() {
        ...
        assert_eq!(manifest["sidecars"][0]["process"]["command"], decl["command"]);
    }
