...
class WorkspaceResourceLimitError(ValueError):
    """Raised when a workspace write would exceed its resource admission."""
...
def workspace_dir(workspace_id: str) -> Path:
    ...
    if workspace_id in {".", ".."} or not re.fullmatch(
        r"[A-Za-z0-9._-]+", workspace_id or ""
    ):
...
def write_file(workspace_id: str, path: str, content: str) -> dict:
    ...
    if len(encoded) > MAX_WORKSPACE_FILE_BYTES:
        raise WorkspaceResourceLimitError(
            f"file content exceeds the {MAX_WORKSPACE_FILE_BYTES}-byte limit"
        )
    current_bytes = sum(
        item.stat().st_size
    ...
    previous_bytes = target.stat().st_size if target.is_file() else 0
    if current_bytes - previous_bytes + len(encoded) > MAX_WORKSPACE_BYTES:
        raise WorkspaceResourceLimitError(
            f"workspace content exceeds the {MAX_WORKSPACE_BYTES}-byte limit"
        )
