...
//! maps [`tool_specs`] onto the registry's `<server>.<tool>` id scheme
//! (`research.run`) so the allowlist, listing, and single `call_tool` entry all
...
use crate::reasoning::{next_variant, CoreCapabilityClient};
...
pub const MAX_WRITE_FILE_BYTES: usize = 1024 * 1024;
...
/// `research.<name>` id scheme), keeping this crate free of that core type.
...
pub fn tool_specs() -> Vec<ResearchToolSpec> {
    vec![
        ...
        tool(
            "init_workspace",
            "Create one durable autoresearch campaign and its git-versioned workspace. Returns campaign_id/workspace_id, mutable_files, program_md, and the RLM history threshold.",
            json!({
                "type": "object",
                "properties": {
                    "experiment": { "type": "string", "description": "Experiment kind id, e.g. 'toy' or 'nanochat'." },
                    "goal": { "type": "string", "description": "The campaign objective and metric to optimize." },
                    "reasoning_threshold_chars": { "type": "integer", "minimum": 1, "description": "Require an RLM analysis before the next variant once serialized history reaches this size." }
                },
                "required": ["experiment"]
            }),
        ),
        tool(
            "list_campaigns",
            "List durable autoresearch campaigns with status, elapsed timestamps, attempt count, and winner score.",
            json!({ "type": "object", "properties": {}, "additionalProperties": false }),
        ),
        tool(
            "get_campaign",
            "Read one campaign with its durable attempt history and whether RLM reasoning is required before the next variant.",
            json!({ "type": "object", "properties": { "campaign_id": ws_prop() }, "required": ["campaign_id"] }),
        ),
        tool(
            "read_file",
        ...
        tool(
            ...
            json!({
                ...
                "properties": {
                    ...
                    "content": { "type": "string", "maxLength": MAX_WRITE_FILE_BYTES, "description": "Full new file content; limited to 1 MiB." }
                },
        ...
        tool(
            "run",
            "Commit the current state and run one proposal-bound experiment attempt under a bounded wall-clock budget. Winner selection, reset, and stop policy remain engine-owned.",
            json!({
                "type": "object",
                "properties": {
                    "workspace_id": ws_prop(),
                    "budget_s": { "type": "integer", "description": "Optional wall-clock cap in seconds; defaults to the experiment's budget_s." },
                    "hypothesis": { "type": "string", "description": "The single testable change this attempt evaluates. Verified RLM proposals require an exact match." },
                    "proposal_id": { "type": "integer", "description": "One-shot proposal returned by next_variant." },
                    "analysis_id": { "type": "integer", "description": "Legacy alias for proposal_id." }
                },
        ...
        tool(
            "keep",
            "Keep an exact successful attempt. The engine rejects a score that does not strictly improve the current winner.",
            json!({ "type": "object", "properties": { "workspace_id": ws_prop(), "attempt_id": { "type": "integer" } }, "required": ["workspace_id"] }),
        ),
        tool(
            "reset",
        ...
        tool(
            ...
            json!({
                ...
                "properties": {
                    ...
                    "score": { "type": "number", "description": "Parsed metric; winner direction comes from the experiment policy (append mode)." },
        ...
        tool(
            "next_variant",
            "Prepare or reuse exactly one proposal for the current campaign snapshot. Oversized histories are analyzed through the bound RLM capability with verified citations and recursion; callers cannot supply the proof.",
            json!({
                "type": "object",
                "properties": { "campaign_id": ws_prop() },
        ...
        tool(
            "finish_campaign",
            "Mark a campaign completed, failed, or cancelled and freeze its elapsed time.",
            json!({
                "type": "object",
                "properties": {
                    "campaign_id": ws_prop(),
...
///
/// `tool` is the bare tool name (already stripped of the `research.` prefix by
...
pub async fn dispatch(client: &reqwest::Client, tool: &str, arguments: Value) -> Result<Value> {
    ...
    match tool {
        ...
        "init_workspace" => {
            ...
            let mut body = json!({ "experiment": experiment });
            copy_if_present(&arguments, &mut body, "goal");
            copy_if_present(&arguments, &mut body, "reasoning_threshold_chars");
            post(client, &format!("{base}/workspace/init"), body).await
        }
        "list_campaigns" => get(client, &format!("{base}/campaigns")).await,
        "get_campaign" => {
            let campaign_id = require_safe_id(&arguments, "campaign_id")?;
            get(client, &format!("{base}/campaigns/{campaign_id}")).await
        }
        "read_file" => {
        ...
        "write_file" => {
            ...
            if content.len() > MAX_WRITE_FILE_BYTES {
                return Ok(json!({
                    "available": true,
                    "code": "resource_limit",
                    "error": "Research file content exceeds the 1 MiB limit"
                }));
            }
            put(
                client,
        ...
        "run" => {
            ...
            copy_if_present(&arguments, &mut body, "hypothesis");
            copy_if_present(&arguments, &mut body, "proposal_id");
            copy_if_present(&arguments, &mut body, "analysis_id");
        ...
        "next_variant" => {
            let campaign_id = require_safe_id(&arguments, "campaign_id")?;
            let broker = CoreCapabilityClient::from_env();
            match next_variant(client, broker.as_deref(), &campaign_id).await {
                Ok(value) => Ok(value),
                Err(error) => Ok(json!({
                    "available": true,
        ...
        "finish_campaign" => {
            ...
            post(
                client,
                &format!("{base}/campaigns/{campaign_id}/finish"),
                json!({ "status": status }),
            )
            .await
        }
...
async fn git_action(
    ...
    copy_if_present(arguments, &mut body, "attempt_id");
    post(client, &format!("{base}/workspace/{ws}/git"), body).await
}

async fn get(client: &reqwest::Client, url: &str) -> Result<Value> {
    match authorize_engine(client.get(url)).send().await {
        Ok(r) => parse(r).await,
...
async fn post(client: &reqwest::Client, url: &str, body: Value) -> Result<Value> {
    match authorize_engine(client.post(url)).json(&body).send().await {
        Ok(r) => parse(r).await,
        Err(e) => Ok(unavailable(format!("research sidecar not reachable: {e}"))),
    }
}
...
async fn put(client: &reqwest::Client, url: &str, body: Value) -> Result<Value> {
    match authorize_engine(client.put(url)).json(&body).send().await {
        Ok(r) => parse(r).await,
        Err(e) => Ok(unavailable(format!("research sidecar not reachable: {e}"))),
    }
}
...
fn require_safe_id(args: &Value, key: &str) -> Result<String> {
...
mod tests {
    ...
    #[test]
    fn lists_twelve_research_tool_specs() {
        ...
        assert!(specs.iter().any(|s| s.name == "get_campaign"));
        assert!(specs.iter().any(|s| s.name == "next_variant"));
    }
